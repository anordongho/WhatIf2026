...
// VC 등록 함수
async function registerVC(vcId: string, holderPublicKey: string, expirationDate: number, vcSchemeHash: string) {
    try {
...
// VC 조회 함수
async function getVCAttributes(vcId: string) {
    try {
...
// VC 유효성 확인 함수
async function isVCValid(vcId: string) {
    try {
...
// VC 업데이트 함수
async function updateVC(vcId: string, newExpirationDate: number, newVcSchemeHash: string) {
    try {
        const tx = await contract.updateVC(vcId, newExpirationDate, newVcSchemeHash);
        await tx.wait();
...
async function main() {
    ...
    // const contract = await factory.deploy({ gasLimit: 10000000 });
