...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const factory = new ethers_1.ethers.ContractFactory(contractABI, contractBytecode, wallet);
        const contract = yield factory.deploy({ gasLimit: 1000000 });
        yield contract.deployed();
        console.log(`Contract deployed at: ${contract.address}`);
        // VC 등록
        console.log('Registering a VC...');
        const vcId = ethers_1.ethers.utils.id('vc1');
        ...
        const vcSchemeHash = ethers_1.ethers.utils.id('scheme1');
        const tx = yield contract.registerVC(vcId, holderPublicKey, expirationDate, vcSchemeHash);
        yield tx.wait();
        console.log(`VC registered with ID: ${vcId}`);
        // VC 속성 조회
        console.log('Getting VC attributes...');
        const vcAttributes = yield contract.getVCAttributes(vcId);
        ...
        // VC 유효성 확인
        const isValid = yield contract.isVCValid(vcId);
